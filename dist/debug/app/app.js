...
(function () {
                          ...
                          $rootScope)
    {
        ...
        $scope.getLegend = function(objects, options){
            options = options || {};
            var selected = false;
            ...
            var legend = objects.map(function (list) {
                ...
                if (options.depth && options.depth > 0){
					if (list.lists) {
						result.lists = $scope.getLegend(list.lists, angular.extend({}, options, {depth: options.depth - 1}));
					}
					if (list.clubs){
						result.clubs = $scope.getLegend(list.clubs, angular.extend({}, options, {depth: options.depth - 1}));
					}
                }
                return result;
            });//.reverse();
        ...
        $scope.prepareChartData = function(data, legendMapping){
            var legends = {};
            var legendsByName = {};
            ...
            Object.keys(legends).forEach(function(index){
                ...
                if (legend.name == 'club'){
                    ...
                    legendMapping[legend.name].lists.filter(function(list){ // list - sport. Отфильтровываем спорты
                        var id = list.id;
                        return Object.keys(legend.sports).some(function(index){return legend.sports[index].sportId == id;});
